...
if __name__ == "__main__":
    ...
    for img_file in os.listdir(image_dir):
        ...
        try:
            ...
            image_docs.append(Document(page_content=content, metadata={"source": img_file}))
        except Exception as e:
            print(f"Captioning failed for {img_file}: {e}")

    print(f"\nExtracted and captioned {len(image_docs)} image(s).")
    ...
    print("You can now ask questions from the PDF including graphs/images.\n")
    while True:
        user_query = input("\nAsk (enter Exit to quit): ")
        if user_query.lower() != "exit":
            result = qa.run(user_query)
            ...
            print("Thanks for using chatPDF with image support.")
            break
